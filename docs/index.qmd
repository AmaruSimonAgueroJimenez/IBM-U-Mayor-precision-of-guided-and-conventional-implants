...
#| fig-height: 10
#| fig-width: 10
#| fig-cap: "Figura 3: Medias de desviaciones según diametro del implante y tipo de cirugía."

# Function to prepare data for mean and confidence interval by implant diameter
prepare_data_diameter <- function(data, var) {
  # Group and calculate mean and confidence intervals by implant diameter and surgery type
  data_by_diameter <- data %>%
    ...
    dplyr::summarize(mean_val = mean({{ var }}, na.rm = TRUE),
              ...
              n = sum(!is.na({{ var }})), .groups = 'drop') %>%
    mutate(se = sd_val / sqrt(n),
           ci_lower = pmax(0, mean_val - qt(0.975, df = n - 1) * se), # Ensure lower CI is non-negative
           ci_upper = mean_val + qt(0.975, df = n - 1) * se)
  
  return(data_by_diameter)
}

# Function to create a plot for means by implant diameter
create_plot_diameter <- function(data, var_name, title, y_label, show_y_label = FALSE) {
  ggplot(data, aes(x = `Diámetro del Implante`, y = mean_val, fill = `Tipo de cirugía`)) +
    ...
    labs(title = title,
         y = if (show_y_label) y_label else NULL,
         x = "Diámetro del Implante") +  # Set x-axis label
    theme_minimal() +
    theme(legend.position = "top")
}

# Prepare data for each deviation
...
# Create each plot with a y-axis label only on one plot
...
#| fig-height: 10
#| fig-width: 10
#| fig-cap: "Figura 4: Medias de desviaciones según longitud del implante y tipo de cirugía."
...
#| fig-height: 10
#| fig-width: 10
#| fig-cap: "Figura 5: Medias de desviaciones según presencia de alguna enfermedad sitémica y tipo de cirugía."

# Function to prepare data for mean and confidence interval by systemic disease
...
# Function to create a plot for means by systemic disease
...
# Prepare data for each deviation by systemic disease
...
# Create plots for each deviation
...
#| fig-height: 6
#| fig-width: 6
#| fig-cap: "Figura 6: Comparación de Medias de Desviaciones según Tipo de Cirugía (Test Mann Whitney)."
...
# Perform the calculations for each variable
variables <- c("Desviación angular", "Desviación coronal", "Desviación apical", "Desviación de profundidad")
...
means_ci_data$P_Value <- rep(p_values, each = 2) # Two entries per variable for each surgery type
...
colnames(cleandata) <- gsub(" ", "_", colnames(cleandata))  # Reemplazar espacios con guiones bajos
colnames(cleandata) <- gsub("á", "a", colnames(cleandata))  # Reemplazar acentos agudos
...
for (indep in variables_independientes) {
    for (dep in variables_dependientes) {
        ...
        # Realizar la prueba Scheirer-Ray-Hare y guardar el resultado sin imprimir
        invisible({
...
# Crear un data frame para resumir los resultados de interacción
...
# Poblar el data frame con los resultados de interacción
for (name in names(resultados)) {
    invisible({
        res <- resultados[[name]]
        # Filtrar la fila de la interacción
...
# Ordenar la tabla por Variable_Dependiente
...
dunn_df %>%
  ...
  tab_caption(caption = "Tabla 6: Resultados de la Prueba de Dunn.")
